...
const MatchPredictionModal = ({ match, onClose, onSubmit, players, isEditing, initialPrediction }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (winningTeam && potm && !isSubmitting) {
      setIsSubmitting(true);
      try {
        await onSubmit(winningTeam, potm);
      } finally {
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-lg w-full mx-4">
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="flex justify-end space-x-3">
            ...
            <button
              type="submit"
              disabled={isSubmitting || !winningTeam || !potm}
              className={`min-w-[160px] px-4 py-2 ${isEditing ? 'bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800' : 'bg-gradient-to-r from-indigo-600 to-purple-700 hover:from-indigo-700 hover:to-purple-800'} text-white rounded shadow-sm focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2`}
            >
              {isSubmitting ? (
